...
**Feature Branch**: `003-enhanced-vault-search`  
**Created**: 2026-04-08  
**Status**: Draft  
...
A user wants to find notes from a specific time period (e.g., "notes from last week" or "meeting notes from January"). The system applies a hard time filter and returns only notes that fall inside the requested window, ordering them by recency.
...
**Independent Test**: Create notes across different dates. Search with a query that includes a time qualifier. Verify only notes from the specified time range are returned, with newer matching notes ranked above older ones.

**Acceptance Scenarios**:

1. **Given** a user searches for "notes from last week", **When** the query is processed, **Then** only notes modified within the last 7 days are returned.
2. **Given** a user searches for "meeting notes from January 2026", **When** results are returned, **Then** notes with modification dates outside January 2026 are excluded and the remaining notes are ordered by recency.
...
- **FR-008**: System MUST chunk documents using header-aware splitting to preserve section context, with configurable sentence overlap when additional continuity is needed.
...
- **FR-011**: System MUST keep indexing and retrieval local to the plugin, with external requests limited to the user's configured embedding provider and any explicitly configured reranking backend.
